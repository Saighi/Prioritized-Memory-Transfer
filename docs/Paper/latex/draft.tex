...
\paragraph{Surprise and free energy.} Each network carries a free energy, a precision-weighted sum of
squared errors,
\begin{equation}
\label{eq:freeenergy}
...
These energies are descriptors of the dynamics, not its generators, and the accounting deserves to be
explicit. In wake ($\piST>0$) every force in the network is the gradient of a bounded free energy and
the model is standard predictive coding. The sleep flip is our one deliberate departure from that
framework: for $\piST<0$ no bounded free energy owns the teacher's interface term, so the
reversed-precision push is postulated rather than derived --- the results that follow then characterize
it exactly (ascent on the student's settled free energy, Theorem~\ref{thm:prioritized}; the negative
half of a single zero-sum potential at $\piST=-\piTS$, Proposition~\ref{prop:saddle}).
...
recurrent prior, which prevents it from confabulating), and a bound on the \emph{magnitude} of the
reversed precision --- we keep $|\piST|$ well below the teacher's self-precision (half the measured
stability threshold throughout), which keeps the teacher on its memory manifold during sleep, as we
verify numerically.
...
provided the reversed precision is not too strong: we operate with $|\piST|$ well below the teacher's
self-precision (half the measured stability threshold throughout), and verify numerically that the
teacher's off-manifold occupancy remains negligible during sleep. This is what sets the
precision-weighting regime: the reversed precision must be strong enough to move the teacher along
novel memory directions, yet weak enough that the self-pull wins normal to the manifold.
...
on-manifold at small $|\piST|$ and abruptly chasing noise once the push overwhelms the self-pull;
our operating point sits well inside the stable regime, at half the measured turnover threshold.
Second, selection is spectral, not a race in time: across the entire
...
justified, in Appendix~\ref{app:proofs}. Proposition~\ref{prop:saddle} (the saddle at $\piST=-\piTS$)
and the on-manifold containment of the teacher at our operating point are established with the
numerical diagnostics below.
...
manifold occupancy across $|\piST|$, both stable at our operating point and turning over only well
above it; the restricted novelty spectrum
...
$|\piST|$ (the stability turnover); spectral, not temporal, selection across $\tauS/\tauT$.
