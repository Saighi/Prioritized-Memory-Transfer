...
| (A + B)ᵀ = Aᵀ + Bᵀ | splits over sums | I − N_S symmetric (Lemma 1 S4: U(I−D_n)Uᵀ) |
| (A⁻¹)ᵀ = (Aᵀ)⁻¹ | transpose & inverse commute | B⁻¹ symmetric (legacy operator route — not in the default proofs) |
| ★ a scalar equals its own transpose: xᵀy = yᵀx, and more generally s = sᵀ for any 1×1 | the trick that merges the two middle terms of a quadratic expansion | Cor 1 (landscape gradient remark) |
| xᵀAy = yᵀAᵀx; if A symmetric, xᵀAy = yᵀAx | "slide A across by transposing it" | prioritization remark (u_kᵀN_S = n_k u_kᵀ) |
...
| A(B + C) = AB + AC, (A+B)C = AC + BC | distributes both ways | Lemma 1 S4 (reassembly), Cor 1 |
...
| ★ one is a polynomial / power / inverse of the other | S_S and B = π_TS I + π_S S_S, hence S_S and B⁻¹ (legacy operator route — not in the default proofs) |
| they share an orthonormal eigenbasis (⟺ AB = BA, both symmetric) | N_S, S_S, (I − N_S): all U·diag·Uᵀ in one frame (Lemma 1 S1, S4) |
...
| ★ A⁻¹A = AA⁻¹ = I | a matrix commutes with **its own** inverse | (legacy operator route — not in the default proofs) |
| invertible ⟺ no zero eigenvalue | det = product of eigenvalues | (legacy operator route — not in the default proofs); default route divides per mode |
| eigen-action of the inverse: Av = λv ⇒ A⁻¹v = (1/λ)v | inverse = "one over" per eigenvalue | Lemma 1 S6 (closed form) |
...
| product AB of two symmetric matrices is symmetric | **iff they commute** | (legacy operator route — not in the default proofs); default: N_S = UD_nUᵀ symmetric by construction |
...
| Fact | Note | Used |
|---|---|---|
| ★ vᵀ(MᵀM)v = ‖Mv‖² ≥ 0 | the **sandwich instinct**: see MᵀM between vᵀ and v, read a squared norm | Lemma 1 S1(iii), Cor 1 |
| hence MᵀM ⪰ 0 always | its eigenvalues are μ = ‖Mv‖²/‖v‖² ≥ 0 | S_S, S_T PSD |
| same kernel: Mx = 0 ⟺ MᵀMx = 0 | learned direction: M_S u = 0 ⇒ S_S u = 0 ⇒ N_S u = 0 | Lemma 1 S1(iii), S6 |
| A + cI shifts every eigenvalue by c | B = π_TS I + π_S S_S has eigenvalues π_TS + π_S μ ≥ π_TS > 0 | Lemma 1 S3 (per-mode divisor) |
| PSD + no zero eigenvalue ⇒ invertible | why B⁻¹ exists | (legacy operator route — not in the default proofs) |
| A ⪯ B means vᵀAv ≤ vᵀBv ∀v | compare eigenvalues in a shared basis | extinction bound N_S ⪯ (π_S/π_TS)S_S (prioritization remark, self-limiting) |
| Rayleigh bounds: λ_min ≤ xᵀAx/‖x‖² ≤ λ_max | eigenvalue sandwiching a quadratic form | (archived guard analysis — not in the paper) |
...
| Fact | Note | Used |
|---|---|---|
| ★ **spectral theorem**: A symmetric ⇒ A = U D Uᵀ — rotation U (orthonormal eigenvector columns, UᵀU = UUᵀ = I), diagonal D (real eigenvalues), rotation back | read right to left: rotate into the eigen-frame, scale each coordinate by its dial, rotate back; products collapse via the middle UᵀU = I | Lemma 1 S1, S4 |
...
| linear ODE decouples: ẏ = Ay, A = UDUᵀ ⇒ c = Uᵀy obeys ċ = Dc ⇒ ċ_k = λ_k c_k ⇒ c_k(t) = c_k(0)e^{λ_k t} | rotate coordinates → middle UᵀU evaporates → diagonal system = d independent scalar ODEs → solve, rotate back; fully worked in Lemma 1, Steps 2–5 (student modes, then the teacher's law) | Lemma 1 S5 (rates |π_ST|n_k) |
...
| ★ quadratic form (A symmetric) | ∇ₓ ½ xᵀAx = Ax | ∇F_S^eq = π_TS N_S x_T (Cor 1, landscape gradient) |
...
| ★ chain rule along a flow | dF/dt = ∇F · ẋ — (G1) with δ = ẋ dt (Frobenius version for matrices: dF/dt = ⟨∇_W F, Ẇ⟩) | Prop 1 S8, Thm 1 Remark 1, prioritization remark |
| ★ Cauchy–Schwarz → steepest ascent | ∇F·e ≤ ‖∇F‖ for unit e, equality iff e ∥ ∇F: **the gradient is the steepest-ascent direction** (standard; one clause in the paper) | Thm 1, closing clause |
| positive rescaling keeps the direction | c > 0 ⇒ c∇F points along ∇F; ascent property unchanged | u = (|π_ST|/π_TS)∇F_S^eq |
| ★ envelope theorem ("free lunch at a minimum") | for g(θ) = min_x f(x, θ): ∇_θ g = ∂f/∂θ evaluated at the minimizer — the minimizer's motion contributes nothing because the slope there is zero. Stated + proved + toy example in Theorem 1, Step 2 of detailed_proofs.md | Thm 1 Step 2 (∇F_S^eq) |
| gradient flow descends | ẋ = −∇F ⇒ dF/dt = −‖∇F‖² ≤ 0 (ascent: flip both signs) | Prop 1, prioritization remark |
...
| P⊥ = I − yyᵀ/‖y‖² | projector onto a sphere's tangent plane (kills the radial part) | Prop 2 (sphere) |
...
(‖Ax‖²), Prop 1 S5 (the weight gradient), Cor 1 landscape gradient (xᵀN_Sx). The ½ in front of squared norms exists
...
Instantly gives PSD, kernels, and per-direction costs. *Used:* Lemma 1 S1(iii), Cor 1.
...
matrix version of "multiply by 1 cleverly." *Used:* (legacy operator route — not in the default proofs).

**P4 — Factor the unknown, then invert (legacy — the default proofs avoid it).**
Collect all terms in the unknown vector into (matrix)·x = rhs, prove the matrix has no zero
eigenvalue (usually: PSD + positive shift, §6), then left-multiply by the inverse. This is "solving
a linear system" in matrix clothing. *Status:* the legacy operator route to Lemma 1; the default
dynamics-first route replaces it entirely with a per-mode division by the positive number
π_TS + π_S μ_k (Lemma 1 S3) — no invertibility argument needed.
...
matrix identity ⟺ one scalar identity for each μ. *Used:* (legacy operator route — not in the default proofs); the default proofs get the same per-mode scalars from the rotation machinery (Lemma 1 S1).

**P7 — Diagonalize the dynamics (rotate coordinates, then solve per axis).**
The full recipe, worked in detail in Lemma 1, Steps 1–5 (rotate, settle each mode, reassemble, teacher's law): (1) spectral theorem
...
yᵀAy = cᵀDc = Σ λ_k c_k² (Cor 1). *Used:* prioritization remark (ċ_k = (|π_ST|/τ_T)n_k c_k), Cor 1.
...
toy check live in Theorem 1, Step 2 of detailed_proofs.md.) *Used:* Thm 1 Step 2 — one line to
∇F_S^eq = π_TS N_S x_T.

**P10 — Complement split (predicted + novel = whole) (legacy — now free, per mode).**
The split x_S* = (I − N_S)x_T ("copied") and ε_TS = −N_S x_T ("missing") used to be extracted from
the operator identity π_TS B⁻¹ = I − N_S. In the default dynamics-first route it costs nothing:
per mode, s_k* = (1 − n_k)c_k and e_k = −n_k c_k come straight out of settling the scalar
tug-of-war (Lemma 1 S3), and the reassembly (Lemma 1 S4) is the vector form. The scalar identity
π_S μ_k(1 − n_k) = π_TS n_k plays the old identity's role in Cor 1.
...
gives exact conditions. *Used:* Prop 1 (definition of the dynamics), Prop 2 (saddle condition
|π_ST| = π_TS).
...
| H5 ★ | x_S* = (I − N_S)x_T — per mode s_k* = (1 − n_k)c_k | attenuated copy: the student settles on what it can predict, mode by mode |
...
| H11 ★ | u = π_ST ε_TS = (|π_ST|/π_TS) ∇F_S (any x_S — the sign identity); settled: = |π_ST| N_S x_T = (|π_ST|/π_TS)∇F_S^eq | the push = the student's gradient, sign-flipped (Thm 1); N_S form = landscape (Cor 1) |
| H12 ★ | τ_T ẋ_T = −∇F_T + (|π_ST|/π_TS)∇F_S^eq; single potential Φ = F_T − F_S iff π_ST = −π_TS | descend own surprise, ascend the student's; exact saddle = mobility matching (Prop 2) |
