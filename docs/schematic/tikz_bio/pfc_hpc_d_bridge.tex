...
\draw[down, excit] (xs) to[bend left=22] node[glab, pos=0.5, anchor=west, xshift=1pt]{ $x_S$ } (ets);
...
\draw[down, inhib] (ets) to[bend left=22] node[glab, pos=0.5, anchor=west, xshift=1pt]{ $\pi_{ST}<0$ } (xt);
...
\node[note, anchor=center] at (4.20,-6.15) { the receiver's reconstruction is returned to the source with a reversed sign:\\[1pt]shared content cancels, the residual survives and is replayed };
